    ...
    <MainLayout title="Agent Manager Detail">
      <div className="mb-6 space-y-4">
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-border bg-white p-5 sm:flex-row sm:items-center sm:justify-between md:p-6">
          ...
          <div className="flex items-center gap-3"><span className={`rounded-full border px-3 py-1.5 text-xs font-bold capitalize ${manager.status === "active" ? "border-[#52C41A] text-[#52C41A]" : manager.status === "pending" ? "border-[#BDF40B] bg-[#BDF40B] text-[#10110E]" : "border-[#FF4D4F] text-[#FF4D4F]"}`}>{manager.status || "inactive"}</span><Button onClick={() => navigate(`/agent-managers/${manager.id}/edit`)} className="h-10 rounded-lg bg-[#BDF40B] text-[#10110E] hover:bg-[#BDF40B]"><Edit2 className="mr-2 h-4 w-4" />Edit</Button></div>
        </div>
      </div>
