...
const getTxMeta = (tx: Transaction, colors: any, t: (key: string) => string) => {
  const type = tx.transaction_type;
  if (type === 'agent_to_customer') return {
    label: t('home.tx_money_received'),
    ...
    try {
      ...
      if (profileRes.status === 200 && profileRes.body?.success) {
        ...
        if (prevBalanceRef.current !== null && newBalance > prevBalanceRef.current) {
          ...
          Toast.show({
            type: "success",
            text1: t('home.money_received'),
            text2: t('home.money_received_desc', { amount: diff.toLocaleString() }),
          });
  ...
  if (loading && !refreshing && !profile) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.background }}>
        ...
        <Text style={{ marginTop: 12, fontSize: 13, color: colors.textSecondary }}>{t('home.loading_dashboard')}</Text>
      </View>
    );
  }
      ...
      >
        ...
        <View style={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 22, fontWeight: '800', color: colors.text, marginTop: 2, letterSpacing: -0.5 }}>
              {profile?.full_name ?? t('home.greeting_user')}
            </Text>
          </View>
          ...
          <View style={{
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 14, fontWeight: '800', color: colors.error }}>
                {t('profile.kyc_rejected_title')}
              </Text>
              <Text style={{ fontSize: 12, color: colors.text, marginTop: 4, lineHeight: 16 }}>
                {t('profile.kyc_rejection_reason', {
                  reason: profile?.nrc_verification?.rejection_reason || t('home.kyc_no_reason'),
                })}
              </Text>
              <Text style={{ fontSize: 11, color: colors.textSecondary, marginTop: 6, fontWeight: '600' }}>
                {t('home.kyc_reverify_hint')}
              </Text>
            </View>
          </View>
        )}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <View style={{
                ...
                <Text style={{ fontSize: 11, fontWeight: '700', color: colors.secondary, textTransform: 'uppercase', letterSpacing: 0.4 }}>
                  {profile?.wallet?.status ?? t('profile.status_active')}
                </Text>
              </View>
            </View>
          </LinearGradient>
        ...
        <View style={{ paddingHorizontal: 24, marginTop: 24 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
            {[
              { label: t('home.send_money'), icon: 'arrow-up-right', color: colors.primary, bg: `${colors.primary}1F`, route: '/cash-in' },
              { label: t('home.my_qr'), icon: 'grid', color: colors.success, bg: `${colors.success}1F`, route: '/qr-code' },
              { label: t('home.scan_qr'), icon: 'camera', color: colors.primary, bg: `${colors.primary}1F`, route: '/cash-in?scan=true' },
            ].map((action) => (
              <TouchableOpacity
                key={action.route}
