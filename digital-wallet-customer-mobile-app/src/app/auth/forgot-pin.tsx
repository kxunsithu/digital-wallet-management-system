...
export default function ForgotPinScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (response.status === 200 && response.body?.success) {
      Toast.show({
        type: 'success',
        text1: t('auth.otp_sent_title'),
        text2: t('auth.otp_sent_desc')
      });
      ...
      Toast.show({
        ...
        text2: response.body?.message ?? t('auth.failed_to_send_otp')
      });
    }
  };
          ...
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24, paddingVertical: 16 }}>
            ...
            >
              <Text style={{ fontSize: 13, color: colors.textSecondary }}>
                {t('auth.remember_pin')} {' '}
                <Text style={{ color: colors.primary, fontWeight: '700' }}>{t('auth.login_btn')}</Text>
              </Text>
            </TouchableOpacity>
