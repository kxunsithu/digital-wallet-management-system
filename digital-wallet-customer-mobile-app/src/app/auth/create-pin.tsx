...
export default function CreatePinScreen() {
  ...
  const handleSubmit = async () => {
    if (step === 'profile') {
      if (!fullName.trim()) {
        Toast.show({ type: 'error', text1: t('common.required'), text2: t('auth.enter_full_name_error') });
        return;
      }
      if (!nrcNumber.trim()) {
        Toast.show({ type: 'error', text1: t('common.required'), text2: t('auth.enter_nrc_error') });
        return;
      }
    ...
    if (confirmString.length !== PIN_LENGTH) { Toast.show({ type: 'error', text1: t('common.error'), text2: t('auth.confirm_pin_error') }); return; }
    ...
    if ((response.status === 201 || response.status === 200) && response.body?.success) {
      Toast.show({ type: 'success', text1: t('auth.pin_created'), text2: t('auth.pin_created_desc') });
      ...
      if (needsFullName || needsNrc || (typeof message === 'string' && message.toLowerCase().includes('full name'))) {
        if (isMounted.current) {
          setStep('profile');
          Toast.show({ type: 'error', text1: t('auth.additional_info_required'), text2: t('auth.additional_info_required_desc') });
        }
        return;
      }

      Toast.show({ type: 'error', text1: t('common.error'), text2: response.body?.message ?? t('auth.failed_to_create_pin') });
    }
  };
