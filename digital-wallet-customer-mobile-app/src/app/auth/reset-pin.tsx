...
export default function ResetPinScreen() {
  ...
  const handleResendOtp = async () => {
    ...
    if (resendCount >= MAX_RESEND_ATTEMPTS) {
      Toast.show({
        ...
        text2: t('auth.max_attempts_reached')
      });
      return;
    }
    ...
    if (response.status === 200 && response.body?.success) {
      ...
      Toast.show({
        type: 'success',
        text1: t('auth.otp_resent'),
        text2: t('auth.otp_resent_desc', { count: resendCount + 1, max: MAX_RESEND_ATTEMPTS })
      });
    } else {
      Toast.show({
        type: 'error',
        text1: t('auth.resend_failed'),
        text2: response.body?.message ?? t('auth.failed')
      });
    }
  };
  ...
  const verifyOtp = async () => {
    ...
    if (otpString.length !== OTP_LENGTH) {
      Toast.show({ type: 'error', text1: t('auth.invalid_code'), text2: t('auth.invalid_otp') });
      return;
    }
    ...
    if (response.status === 200 && response.body?.success) {
      ...
      Toast.show({ type: 'error', text1: t('common.error'), text2: response.body?.message ?? t('auth.invalid_otp') });
    }
  };
  ...
  const handleResetPin = async () => {
    ...
    if (pinString.length !== PIN_LENGTH) {
      Toast.show({ type: 'error', text1: t('common.error'), text2: t('auth.pin_must_4_digits') });
      return;
    }

    if (pinString !== confirmString) {
      Toast.show({ type: 'error', text1: t('auth.pin_mismatch'), text2: t('auth.pin_mismatch_desc') });
    ...
    if (response.status === 200 && response.body?.success) {
      Toast.show({ type: 'success', text1: t('auth.pin_reset'), text2: t('auth.pin_reset_desc') });
      router.push('/auth');
    } else {
      Toast.show({ type: 'error', text1: t('common.error'), text2: response.body?.message ?? t('auth.failed_to_reset_pin') });
    }
  };
